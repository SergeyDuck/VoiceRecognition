package com.example.voicerecognition.base
...
class BasePagination<DATA_API : UniqueId>(
    ...
    private val isCheckUniqueOn: Boolean = false
) {
    ...
    private val listPager
        get() = Pager(
            ...
            pagingSourceFactory = {
                object : PagingSource<Int, DATA_API>() {
                    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, DATA_API> {
                        ...
                        val response =
                            unit(pageCurrent) { hasNextUnit: Boolean?, hasPrevUnit: Boolean?, codeResponseUnit: Int ->
            ...
            .let { flow ->
                if (isCheckUniqueOn) flow.map { map ->
                    val personMap = mutableSetOf<Int>()
                    map.filter { person ->
                        if (personMap.contains(person.id)) false
                        else personMap.add(person.id)
                    }
                }
                else flow
            }
            .let { flow -> scope?.let { flow.cachedIn(scope) } ?: flow }
